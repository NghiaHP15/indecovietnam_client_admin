...
import { deleteRole, internalRoles } from "@/api/services/roleService";
import { Detail, RoleModalProps } from "./detail";
...
const Role = () => {
  ...
  const permission: any = usePermission(ROUTE_NAME.Role);
  ...
  const fetchRole = async () => {
    try{
      setLoading(true);
      const res = await internalRoles({limit: lazyParams.limit, search: debouncedValue});
      if(res){
        setData(res)
      }
    } catch (error) {
  ...
  useEffect(() => {
    setColumns([
      {
        key: 'name',
        title: t('setting.role.field.name'),
        dataIndex: 'name',
        width: 200,
      ...
      },
      {
        ...
        title: t('setting.role.field.description'),
        width: 250,
  ...
  const onDelete = async (formValue: IRoleGroup) => {
    if(!formValue.id) return;
    try {
      setLoading(true);
      const res = await deleteRole(formValue.id);
      if(res) reload();
    } catch (error) {
      console.error(error);
    } finally {
            ...
            <Flex justify="space-between" className="mb-5 p-3 rounded-md" style={{ backgroundColor: colorBgElevated }}>
              ...
              <Space>
                ...
                <Select 
                  ...
                  mode="multiple"
                  maxTagCount={1}
                  options={Object.keys(t('setting.role.field', { returnObjects: true })).map((key) => ({
                    value: key,
                    label: t(`setting.role.field.${key}`),
                  }))}
