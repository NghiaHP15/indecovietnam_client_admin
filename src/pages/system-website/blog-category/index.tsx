...
import { createBlogCategory, deleteBlogCategory, internalBlogCategorys, updateBlogCategory } from "@/api/services/blogCategoryService";

type PropKey = keyof IBlogCategory;
...
const BlogCategory = () => {
  ...
  const isEditing = (record: IBlogCategory) => record.id === editingKey;

  const edit = (record: Partial<IBlogCategory> & { id: React.Key }) => {
  ...
  const fetchBlog = async () => {
    try{
      setLoading(true);
      const res = await internalBlogCategorys({limit: lazyParams.limit, search: debouncedValue});
      if(res){
        setData(res)
      }
    } catch (error) {
  ...
  const columsTable = [
    ...
    {
      key: 'title',
      title: t('website.blog-category.title'),
      dataIndex: 'title',
      width: 200,
      editable: true,
      render: (_: any, record: IBlogCategory) => <span className="line-clamp-2">{record.title}</span>
    },
    {
      key: 'slug',
      dataIndex: 'slug',
      title: t('website.blog-category.slug'),
      hidden: true,
      width: 150,
      render: (_: any, record: IBlogCategory) => <span className="line-clamp-2">{record.slug}</span>
    },
    {
      key: 'description',
      dataIndex: 'description',
      title: t("website.blog-category.description"),
      editable: true,
      width: 250,
      render: (_: any, record: IBlogCategory) => <span className="line-clamp-2">{record.description}</span>
    },
    {
      key: 'action',
  ...
  const mergedColumns: TableProps<IBlogCategory>['columns'] = columns.map((col) => {
    if (!col.editable) {
      return col;
    }
    return {
      ...col,
      onCell: (record: IBlogCategory) => ({
        record,
        inputType: 'text',
  ...
  const save = async (key: React.Key) => {
    try {
      const row = (await form.validateFields()) as IBlogCategory;
      ...
      if (index > -1) {
        const res = await updateBlogCategory(key as string, row);
        if(res) reload();
        setEditingKey('');
      }
    } catch (errInfo) {
  ...
  const onDelete = async (formValue: IBlogCategory) => {
    if(!formValue.id) return;
    try {
      const res = await deleteBlogCategory(formValue.id);
      if(res) reload();
    } catch (error) {
      console.error(error);
    } 
            ...
            <Flex justify="space-between" className="mb-5 p-3 rounded-md" style={{ backgroundColor: colorBgElevated }}>
              ...
              <Space>
                ...
                <Select 
                  ...
                  mode="multiple"
                  maxTagCount={1}
                  options={Object.keys(t('website.blog-category', { returnObjects: true })).map((key) => ({
                    value: key,
                    label: t(`website.blog-category.${key}`),
                  }))}
